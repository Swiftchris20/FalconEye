...
class VideoProcessor(private val context: Context) {
    ...
    private fun processFrameInternalCONTOUR(bitmap: Bitmap): Pair<Bitmap, Bitmap>? {
        ...
        if (boundingRect != null && classificationLabel.isNotEmpty()) {
            ...
            val textColor = when {
                classificationLabel.contains("Speed Type 1",    ignoreCase = true) -> Scalar(8.0,   223.0, 230.0) // Blue
                classificationLabel.contains("Speed Type 2",    ignoreCase = true) -> Scalar(255.0, 255.0,   0.0) // Yellow
                classificationLabel.contains("Speed Type 3", ignoreCase = true) -> Scalar(8.0,   230.0,  74.0) // Green
                classificationLabel.contains("Speed Type 4",      ignoreCase = true) -> Scalar(255.0, 102.0, 102.0) // Red
        ...
        if (Settings.DetectionMode.enableYOLOinference && tfliteInterpreter != null) {
            ...
            YOLOHelper.parseTFLite(out)?.let { bestDetection ->
                ...
                if (Settings.BoundingBox.enableBoundingBox) {
                    ...
                    val combinedLabel = "$ballLabel | $speedLabel"
...
// --------------------------------------------------
object YOLOHelper {
    // Type of ball classes:
    // 1) Define IDs 1…5 instead of 0…5
    ...
    fun drawBoundingBoxes(mat: Mat, box: BoundingBox, classificationLabel: String) {
        ...
        // Prepare label
        val label = "${classificationLabel.ifEmpty { "Tracking" }}"
        val fontScale = 1.5
        val thickness = 2
